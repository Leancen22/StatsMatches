...
// Estructura local de jugador en la cancha
type LivePlayer = {
...
export default function LiveMatchPage() {
  ...
  const [playerOut, setPlayerOut] = useState<number | null>(null) // matchPlayerId
  const [playerIn, setPlayerIn] = useState<number | null>(null)   // matchPlayerId
  ...
  useEffect(() => {
    ...
    const fetchMatch = async () => {
      try {
        ...
        // Transformamos matchPlayers en tu estructura local
        // Por defecto, "playing" = true si el starter era true...
        // Sin embargo, no guardamos "starter" en la DB en este ejemplo. 
        // Decide tú la lógica. Aquí supongamos a todos "playing" = false al inicio:
        const livePlayers: LivePlayer[] = match.matchPlayers.map((mp) => ({
          ...
          playing: false, // ajusta según quieras
          stats: {
            goals: mp.goals,
            assists: mp.assists,
            saves: mp.saves,
  ...
  // 2) Función para iniciar/pausar cronómetro
  const toggleTimer = () => {
    if (isRunning) {
      ...
      intervalRef.current = setInterval(() => {
        ...
        // Sumar 1 seg a todos los players playing
        setPlayers((prev) =>
          prev.map((pl) =>
            pl.playing
              ? {
                  ...pl,
                  stats: { ...pl.stats, playTime: pl.stats.playTime + 1 },
                }
              : pl
          )
        )
  ...
  // 5) Sustitución
  //   Recibimos matchPlayerId de "playerOut" y "playerIn".
  ...
  const saveMatch = async () => {
    ...
    try {
      // Recolectamos las stats finales para enviarlas
      ...
      <header className="border-b bg-background z-10">
        <div className="container flex h-16 items-center px-4">
          ...
          <div className="ml-auto flex items-center gap-4">
            <div className="text-lg font-medium">{matchInfo.opponent && `vs ${matchInfo.opponent}`}</div>
            <Button onClick={toggleTimer} variant={isRunning ? "destructive" : "default"} size="lg" className="text-lg h-12">
            ...
            <Card className="col-span-2">
              ...
              <CardContent className="flex justify-between items-center">
                <div className="text-lg">
                  <div>
                    Jugadores en cancha: <span className="font-bold">{playingPlayers.length}</span>
                  </div>
                  <div>
                    Jugadores en banca: <span className="font-bold">{benchPlayers.length}</span>
                  </div>
                </div>
                      ...
                      <TableCell>
                        <div className="flex justify-center space-x-3">
                          <Button
                            ...
                            onClick={() => incrementStat(player.matchPlayerId, "goals")}
                          >
                          ...
                          <Button
                            ...
                            size="icon"
                            className="h-12 w-12 rounded-full"
                            onClick={() => incrementStat(player.matchPlayerId, "assists")}
                          >
                          ...
                          <Button
                            ...
                            size="icon"
                            className="h-12 w-12 rounded-full"
                            onClick={() => incrementStat(player.matchPlayerId, "saves")}
                          >
                          ...
                          <Button
                            ...
                            size="icon"
                            className="h-12 w-12 rounded-full"
                            onClick={() => incrementStat(player.matchPlayerId, "turnovers")}
                          >
